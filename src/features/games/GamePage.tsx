...
export function GamePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Difficulty filter — shown here for all game types except flashcard (which has it inline) */}
      {type !== 'flashcard' && (
        <div className="mb-5">
      ...
      {type === 'timed' && <TimedGame key={gameKey} {...commonProps} />}
      {type === 'pairs' && <PairsGame key={gameKey} {...commonProps} />}
      {type === 'test' && <TestGame key={gameKey} {...commonProps} />}
      {type === 'write' && <WriteGame key={gameKey} {...commonProps} />}
      {type === 'parts' && <PartsGame key={gameKey} {...commonProps} />}
    </div>
  )
}
