...
function buildColumns(batch: Content[]): { questions: PairCell[]; answers: PairCell[] } {
  const questions: PairCell[] = shuffle(batch.map((c) => ({
    cellId: `q-${c.id}`,
    cardId: c.id,
    text: c.question,
    matched: false,
  })))
  const answers: PairCell[] = shuffle(batch.map((c) => ({
    cellId: `a-${c.id}`,
    cardId: c.id,
    text: c.answer,
    matched: false,
...
export function PairsGame({ cards: allCards, onPlayAgain, onRetryMistakes, onBack }: Props) {
  ...
  const [questions, setQuestions] = useState<PairCell[]>(() =>
    buildColumns(deck.slice(0, BATCH_SIZE)).questions
  )
  const [answers, setAnswers] = useState<PairCell[]>(() =>
    buildColumns(deck.slice(0, BATCH_SIZE)).answers
  )
  ...
  function getCellClass(cell: PairCell): string {
    ...
    if (cell.matched)
      return 'bg-green-50 border-green-200 text-green-500 opacity-40 cursor-default'
    if (isWrong)
      return 'bg-red-50 border-red-400 text-red-600 animate-pulse'
    if (isSelected)
      return 'bg-indigo-50 border-indigo-400 text-indigo-700 scale-105'
