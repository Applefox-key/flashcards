...
function ForgotForm({ onBack }: { onBack: () => void }) {
  ...
  if (sent) {
    return (
      <div className="flex flex-col gap-4 text-center">
        ...
        <button onClick={onBack} className="text-sm text-indigo-600 hover:underline mt-2">
          ← Back to login
        </button>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button type="button" onClick={onBack} className="text-sm text-gray-500 hover:text-gray-700 text-center">
        ← Back to login
      </button>
    </form>
  );
...
export function AuthPage() {
  ...
  const INPUT_CLS =
    "border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500";
      ...
      <div className="bg-white rounded-lg shadow-md w-full max-w-sm p-8">
        ...
        <div className="text-center mb-6">
          <button
            ...
            className="text-sm bg-amber-50 border border-amber-300 text-amber-700 hover:bg-amber-100 transition-colors px-4 py-1.5 rounded-full font-medium">
        ...
        {tab === "forgot" && <h2 className="text-base font-semibold text-gray-800 text-center mb-6">Password reset</h2>}
        ...
        {tab === "login" && (
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              ...
              className="text-xs text-gray-400 hover:text-indigo-600 text-center transition-colors">
              Forgot password?
            </button>
          </form>
        )}
        ...
        {tab === "register" && (
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            ...
            <input name="email" type="email" placeholder="Email" required className={INPUT_CLS} />
            <input name="password" type="password" placeholder="Password" required className={INPUT_CLS} />
            {registerMutation.isError && <p className="text-sm text-red-600">Registration failed. Please try again.</p>}
