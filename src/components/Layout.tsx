...
function NavAvatar({ name, img, token }: { name: string; img?: string; token: string | null }) {
  ...
  return avatarUrl ? (
    ...
    <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-xs font-bold text-indigo-600 select-none">
      {initials}
    </div>
  );
}
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <header className="shrink-0 bg-white border-b border-gray-200 z-10">
        <div className="h-14 flex items-center px-4 gap-4">
          ...
          <div className="ml-auto flex items-center sm:justify-end gap-3">
            {/* Apps switcher */}
            ...
            <NavLink
              to="/about"
              className="hidden sm:inline flex items-center text-gray-400 hover:text-gray-600"
              title="About">
        ...
        <nav className="sm:hidden flex px-2">
          <NavLink
            to="/library"
            className={({ isActive }) =>
              `flex-1 text-center py-2 text-xs font-medium ${isActive ? "text-indigo-700" : "text-gray-500"}`
            }>
            Library
          </NavLink>
          <NavLink
          ...
          <NavLink
            to="/tags"
            className={({ isActive }) =>
              `flex-1 text-center py-2 text-xs font-medium ${isActive ? "text-violet-700" : "text-gray-500"}`
            }>
            Tags
          </NavLink>{" "}
          <NavLink to="/about" className="flex items-center text-gray-400 hover:text-gray-600" title="About">
            <svg
              className="sm:hidden"
              width="18"
              height="18"
